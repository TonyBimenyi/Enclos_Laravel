...
<div class="container_accueil">
            <div class="top_bar_buttons">
                ...
                <a href="{{ route('list_depense.index') }}">
                ...
                <a href="{{ route('types_depense.index') }}">
                ...
                <div class="code">
                    {{   $types = DB::table('type_depenses')->orderBy('id','desc')->get(); }}
                </div>

            </div>
        </div>
        <div class="form-title">
            <h1>Ajouter depenses {{ $dep->id }}ee</h1>
        </div>
                <div class="form">

            <div class="form_vrai">
                <form action="/depense/{{ $dep->id }}" method="POST">
                    @csrf
                    @method('PUT')
                   ...
                   <!-- <input type="text" value="" name="type" ><br><br>-->
                    <select name="type" id="">
                        @foreach ($types as $ty)
                            <option value="{{ $ty->type_depense }}">{{ $ty->type_depense }}</option>
                        @endforeach
                    </select><br><br>
                    <label for="">Montant:</label><br>
                    <input type="number" value="{{ $dep->montant_depense}}" name="montant"  ><br><br>

                    <label for="">Date:</label><br>
                    <input type="date" required="[0-9]" name="date" value="{{ $dep->date_depense }}"><br><br>
                    <label for="">Description:</label><br>
                    <textarea name="description" id="" placeholder="Votre texte ici" >{{ $dep->description_depense }}</textarea><br><br>
